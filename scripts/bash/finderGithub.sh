...
if [ "$regNum" -eq 2 ]; then
  ...
  stringToSearch8="eth.sign"
  stringToSearch9="personal.sign"
  ...
  if [ "$regNum" -eq 1 ]; then
    {
      ...
      echo "~~~~~~~~~~~~~~~~~~ $stringToSearch8 ~~~~~~~~~~~~~~~~~~"
      git grep -I -n -i -o -E -w "$stringToSearch8" -- "$fileRegex" "$fileRegexExclude" "$fileRegexExclude2"
      echo "~~~~~~~~~~~~~~~~~~ $stringToSearch9 ~~~~~~~~~~~~~~~~~~"
      git grep -I -n -i -o -E -w "$stringToSearch9" -- "$fileRegex" "$fileRegexExclude" "$fileRegexExclude2"
